...
def adjust_learning_rate(new_lr, optimizer):
    """Sets the learning rate to the provided new_lr

    Inputs:
        new_lr: (float) new desired learning rate for backpropagation
        optimizer: Pytorch optimizer used for backpropagation
    """
...
def centercrop(tensor, size):
    ...
    if input_dims == 4:
        ...
        raise ValueError(
            "Input tensor for centercrop() must be 2-4 dimensional, instead got tensor of dim {}".format(input_dims))
